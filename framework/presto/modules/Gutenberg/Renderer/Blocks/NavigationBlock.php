...
class NavigationBlock extends AbstractBlock
{
    public function render(array $context): string
    {
        ...
        if (isset($layout['orientation']) && $layout['orientation'] === 'vertical') {
            $navClasses[] = 'is-vertical';
        }
        
        if (!empty($this->classes)) {
            $navClasses = array_merge($navClasses, $this->classes);
        }
        
        $classAttr = ' class="' . implode(' ', array_unique($navClasses)) . '"';
        ...
        // Add aria-label if set
        $ariaLabel = '';
        if (isset($this->attrs['ariaLabel']) && !empty($this->attrs['ariaLabel'])) {
            $ariaLabel = ' aria-label="' . htmlspecialchars($this->attrs['ariaLabel']) . '"';
        }
        ...
        if ($isResponsive) {
            ...
            return "<nav{$classAttr}{$styleAttr}{$interactivityAttrs}{$ariaLabel}>{$responsiveButton}{$modal}</nav>";
        }

        return "<nav{$classAttr}{$styleAttr}{$interactivityAttrs}{$ariaLabel}>{$wrappedInner}</nav>";
    }
}
